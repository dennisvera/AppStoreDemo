...
class SearchCollectionViewController: UICollectionViewController, UISearchBarDelegate {
  ...
  private func fetchItunesApps() {
    ServiceClient.shared.fetchAppsWith(searchTerm: instagram) { [weak self] search, error in
  ...
  func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
    ...
    timer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: false, block: { _ in
      ServiceClient.shared.fetchAppsWith(searchTerm: searchText) { [weak self] search, error in
        guard let strongSelf = self else {return }
        if let error = error {
          print("Failed to Fetch Apps: ", error)
          return
